...
describe('ResolverProxy Tests', () => {
    ...
    let resolver_2;
    let diamondFacet;
    let accessControlImpl;
    let pauseImpl;
    let signer_A;
    let account_A;
    async function deployContracts() {
        resolver = await deployResolver();
        diamondFacet = await new _typechain_1.DiamondFacet__factory(signer_A).deploy();
        accessControlImpl = await new _typechain_1.AccessControlFacet__factory(signer_A).deploy();
        pauseImpl = await new _typechain_1.PauseFacet__factory(signer_A).deploy();
    }
    async function setUpResolver(businessLogicsRegistryDatas, configID, resolverContract) {
        if (!configID)
            configID = CONFIG_ID;
        if (!resolverContract)
    ...
    async function deployResolver() {
        let newResolver = await (await hardhat_1.ethers.getContractFactory('BusinessLogicResolver')).deploy();
        newResolver = newResolver.connect(signer_A);
        await newResolver.initialize_BusinessLogicResolver();
        return newResolver;
    }
    async function checkFacets(businessLogicsRegistryDatas, diamondLoupe) {
        const expectedFacets = await Promise.all(businessLogicsRegistryDatas.map(async (data) => {
            const staticFunctionSelectors = await hardhat_1.ethers.getContractAt('IStaticFunctionSelectors', data.businessLogicAddress);
            return {
        ...
        (0, common_1.assertObject)(await diamondLoupe.getFacets(), expectedFacets);
        ...
        for (const facet of expectedFacets) {
            (0, chai_1.expect)(await diamondLoupe.getFacetSelectors(facet.id)).to.deep.equal(facet.selectors);
            (0, chai_1.expect)(await diamondLoupe.getFacetIdBySelector(facet.selectors[0])).to.deep.equal(facet.id);
            (0, common_1.assertObject)(await diamondLoupe.getFacet(facet.id), facet);
            (0, chai_1.expect)(await diamondLoupe.getFacetAddress(facet.selectors[0])).to.deep.equal(facet.addr);
        }
        (0, chai_1.expect)(await diamondLoupe.getFacetIds()).to.deep.equal(expectedFacetIds);
        (0, chai_1.expect)(await diamondLoupe.getFacetAddresses()).to.deep.equal(expectedFacetAddresses);
    }
    beforeEach(async () => {
        ;
        [signer_A] = await hardhat_1.ethers.getSigners();
    ...
    it('GIVEN deployed facets WHEN deploy a new resolverProxy with correct configuration THEN a new resolverProxy proxy was deployed', async () => {
        ...
        const resolverProxy = await (await hardhat_1.ethers.getContractFactory('ResolverProxy')).deploy(resolver.address, CONFIG_ID, 1, []);
        const diamondCut = await hardhat_1.ethers.getContractAt('DiamondCutFacet', resolverProxy.address);
        const result = await diamondCut.getConfigInfo();
        (0, chai_1.expect)(result.resolver_).to.equal(resolver.address);
        (0, chai_1.expect)(result.configurationId_).to.equal(CONFIG_ID);
        (0, chai_1.expect)(result.version_).to.equal(1);
        const diamondLoupe = await hardhat_1.ethers.getContractAt('DiamondLoupeFacet', resolverProxy.address);
    ...
    it('GIVEN deployed facets WHEN deploying a resolverProxy and registering Facets to use a non exposed signature THEN raise FunctionNotFound and it is not recognized by supportsInterface', async () => {
        ...
        await setUpResolver(businessLogicsRegistryDatas);
        const resolverProxy = await (await hardhat_1.ethers.getContractFactory('ResolverProxy')).deploy(resolver.address, CONFIG_ID, 1, []);
        const accessControl = await hardhat_1.ethers.getContractAt('AccessControl', resolverProxy.address);
        const diamondLoupe = await hardhat_1.ethers.getContractAt('DiamondLoupeFacet', resolverProxy.address);
        const GRANT_ROLE_SIGNATURE = '0x2f2ff15d';
        await (0, chai_1.expect)(accessControl.grantRole(_scripts_1.DEFAULT_ADMIN_ROLE, account_A))
            .to.be.revertedWithCustomError(resolverProxy, 'FunctionNotFound')
            .withArgs(GRANT_ROLE_SIGNATURE);
        await (0, chai_1.expect)(await diamondLoupe.supportsInterface(GRANT_ROLE_SIGNATURE))
            .to.be.false;
    });
    it('GIVEN deployed facets WHEN deploy a diamond to latestVersion and one to a specific version THEN only the latest version one will get updated', async () => {
        ...
        const resolverProxy_v1 = await (await hardhat_1.ethers.getContractFactory('ResolverProxy')).deploy(resolver.address, CONFIG_ID, 1, []);
        const resolverProxy_latest = await (await hardhat_1.ethers.getContractFactory('ResolverProxy')).deploy(resolver.address, CONFIG_ID, 0, []);
        const diamondFacet_v1 = await hardhat_1.ethers.getContractAt('DiamondFacet', resolverProxy_v1.address);
        const diamondFacet_latest = await hardhat_1.ethers.getContractAt('DiamondFacet', resolverProxy_latest.address);
    ...
    it('GIVEN resolverProxy and non-admin user WHEN updating version THEN fails with AccountHasNoRole', async () => {
        ...
        const resolverProxy = await (await hardhat_1.ethers.getContractFactory('ResolverProxy')).deploy(resolver.address, CONFIG_ID, 1, []);
        const diamondCut = await hardhat_1.ethers.getContractAt('DiamondCutFacet', resolverProxy.address);
        await (0, chai_1.expect)(diamondCut.updateConfigVersion(0)).to.be.rejectedWith('AccountHasNoRole');
    });
    it('GIVEN resolverProxy and admin user WHEN updating to non existing version THEN fails with ResolverProxyConfigurationNoRegistered', async () => {
        ...
        const rbac = [
            {
                role: _scripts_1.DEFAULT_ADMIN_ROLE,
                members: [account_A],
            },
        ];
        const resolverProxy = await (await hardhat_1.ethers.getContractFactory('ResolverProxy')).deploy(resolver.address, CONFIG_ID, 1, rbac);
        let diamondCut = await hardhat_1.ethers.getContractAt('DiamondCutFacet', resolverProxy.address);
        diamondCut = diamondCut.connect(signer_A);
        await (0, chai_1.expect)(diamondCut.updateConfigVersion(100)).to.be.rejectedWith('ResolverProxyConfigurationNoRegistered');
    });
    it('GIVEN resolverProxy and admin user WHEN updating version THEN succeeds', async () => {
        ...
        const oldVersion = 1;
        const resolverProxy = await (await hardhat_1.ethers.getContractFactory('ResolverProxy')).deploy(resolver.address, CONFIG_ID, oldVersion, rbac);
        let diamondCut = await hardhat_1.ethers.getContractAt('DiamondCutFacet', resolverProxy.address);
        let result = await diamondCut.getConfigInfo();
        (0, chai_1.expect)(result.resolver_).to.equal(resolver.address);
        (0, chai_1.expect)(result.configurationId_).to.equal(CONFIG_ID);
        (0, chai_1.expect)(result.version_).to.equal(oldVersion);
        diamondCut = diamondCut.connect(signer_A);
        const newVersion = 0;
        ...
        (0, chai_1.expect)(result.resolver_).to.equal(resolver.address);
        (0, chai_1.expect)(result.configurationId_).to.equal(CONFIG_ID);
        (0, chai_1.expect)(result.version_).to.equal(newVersion);
    });
    it('GIVEN resolverProxy and non-admin user WHEN updating configID THEN fails with AccountHasNoRole', async () => {
        ...
        const resolverProxy = await (await hardhat_1.ethers.getContractFactory('ResolverProxy')).deploy(resolver.address, CONFIG_ID, 1, []);
        const diamondCut = await hardhat_1.ethers.getContractAt('DiamondCutFacet', resolverProxy.address);
        await (0, chai_1.expect)(diamondCut.updateConfig(CONFIG_ID_2, 1)).to.be.rejectedWith('AccountHasNoRole');
    });
    it('GIVEN resolverProxy and admin user WHEN updating to non existing configID THEN fails with ResolverProxyConfigurationNoRegistered', async () => {
        ...
        let diamondCut = await hardhat_1.ethers.getContractAt('DiamondCutFacet', resolverProxy.address);
        diamondCut = diamondCut.connect(signer_A);
        await (0, chai_1.expect)(diamondCut.updateConfig(CONFIG_ID_2, 1)).to.be.rejectedWith('ResolverProxyConfigurationNoRegistered');
    });
    it('GIVEN resolverProxy and admin user WHEN updating configID THEN succeeds', async () => {
        ...
        result = await diamondCut.getConfigInfo();
        (0, chai_1.expect)(result.resolver_).to.equal(resolver.address);
        (0, chai_1.expect)(result.configurationId_).to.equal(CONFIG_ID_2);
        (0, chai_1.expect)(result.version_).to.equal(newVersion);
    });
    it('GIVEN resolverProxy and non-admin user WHEN updating resolver THEN fails with AccountHasNoRole', async () => {
        ...
        const resolverProxy = await (await hardhat_1.ethers.getContractFactory('ResolverProxy')).deploy(resolver.address, CONFIG_ID, 1, []);
        const diamondCut = await hardhat_1.ethers.getContractAt('DiamondCutFacet', resolverProxy.address);
        await (0, chai_1.expect)(diamondCut.updateResolver(resolver_2.address, CONFIG_ID_2, 1)).to.be.rejectedWith('AccountHasNoRole');
    });
    it('GIVEN resolverProxy and admin user WHEN updating to non existing resolver THEN fails with ResolverProxyConfigurationNoRegistered', async () => {
        ...
        let diamondCut = await hardhat_1.ethers.getContractAt('DiamondCutFacet', resolverProxy.address);
        diamondCut = diamondCut.connect(signer_A);
        await (0, chai_1.expect)(diamondCut.updateResolver(resolver_2.address, CONFIG_ID_2, 1)).to.be.rejectedWith('ResolverProxyConfigurationNoRegistered');
    });
    it('GIVEN resolverProxy and admin user WHEN updating resolver THEN succeeds', async () => {
        ...
        result = await diamondCut.getConfigInfo();
        (0, chai_1.expect)(result.resolver_).to.equal(resolver_2.address);
        (0, chai_1.expect)(result.configurationId_).to.equal(CONFIG_ID_2);
        (0, chai_1.expect)(result.version_).to.equal(newVersion);
    });
});
