...
describe('Time Travel Tests', () => {
    let factory, businessLogicResolver, diamond, timeTravelFacet;
    let signer_A;
    let account_A;
    ...
    it('GIVEN succesful deployment THEN chainId is hardhat network id', async () => {
        const chainId = 1337;
        await (0, chai_1.expect)(timeTravelFacet.checkBlockChainid(chainId)).to.not.be
            .reverted;
    });
    it('GIVEN new system timestamp THEN change succeeds', async () => {
        const newTimestamp = (0, _scripts_1.dateToUnixTimestamp)('2030-01-01T00:00:00Z');
        const oldSystemTime = 0;
        await (0, chai_1.expect)(timeTravelFacet.changeSystemTimestamp(newTimestamp))
            .to.emit(timeTravelFacet, 'SystemTimestampChanged')
            .withArgs(oldSystemTime, newTimestamp);
        (0, chai_1.expect)(await timeTravelFacet.blockTimestamp()).to.be.equal(newTimestamp);
    });
    it('GIVEN incorrect system timestamp change THEN revert with InvalidTimestamp', async () => {
        const newTimestamp = 0;
        await (0, chai_1.expect)(timeTravelFacet.changeSystemTimestamp(newTimestamp)).to.revertedWithCustomError(timeTravelFacet, 'InvalidTimestamp');
    });
    it('GIVEN system timestamp reset THEN use network timestamp', async () => {
        const newTimestamp = (0, _scripts_1.dateToUnixTimestamp)('2030-01-01T00:00:00Z');
        await timeTravelFacet.changeSystemTimestamp(newTimestamp);
        await (0, chai_1.expect)(timeTravelFacet.resetSystemTimestamp()).to.emit(timeTravelFacet, 'SystemTimestampReset');
        const latestBlock = await hardhat_1.ethers.provider.getBlock('latest');
        const latestTimestamp = latestBlock.timestamp;
        (0, chai_1.expect)(await timeTravelFacet.blockTimestamp()).to.be.equal(latestTimestamp);
    });
});
