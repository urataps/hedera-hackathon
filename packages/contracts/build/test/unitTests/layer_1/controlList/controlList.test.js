...
describe('Control List Tests', () => {
    ...
    beforeEach(async () => {
        ...
        controlListFacet = await hardhat_1.ethers.getContractAt('ControlList', diamond.address);
        pauseFacet = await hardhat_1.ethers.getContractAt('Pause', diamond.address);
    });
    it('GIVEN an initialized contract WHEN trying to initialize it again THEN transaction fails with AlreadyInitialized', async () => {
        await (0, chai_1.expect)(controlListFacet.initialize_ControlList(true)).to.be.rejectedWith('AlreadyInitialized');
    });
    it('GIVEN an account without controlList role WHEN addToControlList THEN transaction fails with AccountHasNoRole', async () => {
        ...
        // add to list fails
        await (0, chai_1.expect)(controlListFacet.addToControlList(account_D)).to.be.rejectedWith('AccountHasNoRole');
    });
    it('GIVEN an account without controlList role WHEN removeFromControlList THEN transaction fails with AccountHasNoRole', async () => {
        ...
        await (0, chai_1.expect)(controlListFacet.removeFromControlList(account_D)).to.be.rejectedWith('AccountHasNoRole');
    });
    it('GIVEN a paused Token WHEN addToControlList THEN transaction fails with TokenIsPaused', async () => {
        // Granting Role to account C and Pause
        await (0, common_1.grantRoleAndPauseToken)(accessControlFacet, pauseFacet, _scripts_1.CONTROL_LIST_ROLE, signer_A, signer_B, account_C);
        // Using account C (with role)
        controlListFacet = controlListFacet.connect(signer_C);
        // add to list fails
        await (0, chai_1.expect)(controlListFacet.addToControlList(account_D)).to.be.rejectedWith('TokenIsPaused');
    });
    it('GIVEN a paused Token WHEN removeFromControlList THEN transaction fails with TokenIsPaused', async () => {
        ...
        controlListFacet = controlListFacet.connect(signer_C);
        // remove from list fails
        await (0, chai_1.expect)(controlListFacet.removeFromControlList(account_D)).to.be.rejectedWith('TokenIsPaused');
    });
    it('GIVEN an account with controlList role WHEN addToControlList and removeFromControlList THEN transaction succeeds', async () => {
        ...
        (0, chai_1.expect)(listCount).to.equal(2);
        (0, chai_1.expect)(listMembers.length).to.equal(listCount);
        (0, chai_1.expect)(listMembers[0].toUpperCase()).to.equal(account_D.toUpperCase());
        (0, chai_1.expect)(listMembers[1].toUpperCase()).to.equal(account_E.toUpperCase());
        // REMOVE FROM LIST ------------------------------------------------------------------
        // remove From list
        await (0, chai_1.expect)(controlListFacet.removeFromControlList(account_D))
        ...
        (0, chai_1.expect)(listMembers.length).to.equal(listCount);
        (0, chai_1.expect)(listMembers[0].toUpperCase()).to.equal(account_E.toUpperCase());
        // CHECK LIST TYPE ------------------------------------------------------------------
        const listType = await controlListFacet.getControlListType();
        (0, chai_1.expect)(listType).to.equal(false);
    });
});
