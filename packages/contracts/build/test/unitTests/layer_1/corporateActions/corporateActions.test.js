...
describe('Corporate Actions Tests', () => {
    ...
    async function setFacets(diamond) {
        accessControlFacet = _typechain_1.AccessControlFacet__factory.connect(diamond.address, signer_A);
        corporateActionsFacet = _typechain_1.CorporateActions__factory.connect(diamond.address, signer_A);
        pauseFacet = _typechain_1.PauseFacet__factory.connect(diamond.address, signer_A);
    }
    ...
    it('GIVEN an account without corporateActions role WHEN addCorporateAction THEN transaction fails with AccountHasNoRole', async () => {
        ...
        // add to list fails
        await (0, chai_1.expect)(corporateActionsFacet.addCorporateAction(actionType, actionData)).to.be.rejectedWith('AccountHasNoRole');
    });
    it('GIVEN a paused Token WHEN addCorporateAction THEN transaction fails with TokenIsPaused', async () => {
        // Granting Role to account C and Pause
        await (0, common_1.grantRoleAndPauseToken)(accessControlFacet, pauseFacet, _scripts_1.CORPORATE_ACTION_ROLE, signer_A, signer_B, account_C);
        // Using account C (with role)
        corporateActionsFacet = corporateActionsFacet.connect(signer_C);
        // add to list fails
        await (0, chai_1.expect)(corporateActionsFacet.addCorporateAction(actionType, actionData)).to.be.rejectedWith('TokenIsPaused');
    });
    it('GIVEN an account with corporateActions role WHEN addCorporateAction THEN transaction succeeds', async () => {
        ...
        (0, chai_1.expect)(listMembers[0]).to.equal(corporateActionId_1);
        (0, chai_1.expect)(listCountByType).to.equal(1);
        (0, chai_1.expect)(listMembersByType.length).to.equal(listCountByType);
        (0, chai_1.expect)(listMembersByType[0]).to.equal(corporateActionId_1);
        (0, chai_1.expect)(corporateAction[0].toUpperCase()).to.equal(actionType.toUpperCase());
        (0, chai_1.expect)(corporateAction[1].toUpperCase()).to.equal(actionData.toUpperCase());
    });
});
