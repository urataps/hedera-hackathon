...
async function contractCall(contractId, functionName, parameters, clientOperator, gas, abi, value = 0) {
    const functionCallParameters = encodeFunctionCall(functionName, parameters, abi);
    const contractTx = await new sdk_1.ContractExecuteTransaction()
...
function createContractFactory(factory) {
    return new ethers_1.ContractFactory(factory.createInterface(), factory.bytecode);
}
