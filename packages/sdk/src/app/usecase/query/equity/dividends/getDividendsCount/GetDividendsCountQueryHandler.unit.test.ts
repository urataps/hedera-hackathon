...
describe('GetDividendsCountQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetDividendsCountQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetDividendsCountQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetDividendsCountQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying dividends count: ${errorMsg}`,
        ),
    ...
    it('should successfully get dividends count', async () => {
      ...
      expect(queryAdapterServiceMock.getDividendsCount).toHaveBeenCalledWith(
        evmAddress,
      );
    });
  });
