...
export class GetTotalDividendHoldersQueryHandler
  implements IQueryHandler<GetTotalDividendHoldersQuery>
{
  constructor(
  ...
  async execute(
    query: GetTotalDividendHoldersQuery,
  ): Promise<GetTotalDividendHoldersQueryResponse> {
    try {
      ...
      const res = await this.queryAdapter.getTotalDividendHolders(
        securityEvmAddress,
        dividendId,
      );
