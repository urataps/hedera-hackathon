...
describe('GetDividendsQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetDividendsQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetDividendsQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetDividendsQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying dividends: ${errorMsg}`,
        ),
    ...
    it('should successfully get dividends', async () => {
      ...
      expect(queryAdapterServiceMock.getDividends).toHaveBeenCalledWith(
        evmAddress,
        query.dividendId,
      );
    });
