...
describe('GetScheduledBalanceAdjustmentQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetScheduledBalanceAdjustmentQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetScheduledBalanceAdjustmentQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying scheduled balance adjustment: ${errorMsg}`,
        ),
    ...
    it('should successfully get scheduled balance adjustment', async () => {
      ...
      queryAdapterServiceMock.getScheduledBalanceAdjustment.mockResolvedValue(
      ...
      expect(
        queryAdapterServiceMock.getScheduledBalanceAdjustment,
      ).toHaveBeenCalledWith(evmAddress, query.balanceAdjustmentId);
    });
  });
});
