...
describe('GetVotingForQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetVotingForQueryHandler(
      contractServiceMock,
  ...
  describe('execute', () => {
    it('throws GetVotingForQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetVotingForQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying account's voting: ${errorMsg}`,
        ),
    ...
    it('should successfully get voting for', async () => {
      ...
      expect(queryAdapterServiceMock.getVotingFor).toHaveBeenCalledWith(
        evmAddress,
        targetEvmAddress,
        query.votingId,
      );
