...
describe('GetVotingCountQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetVotingCountQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetVotingCountQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetVotingCountQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying voting count: ${errorMsg}`,
        ),
    ...
    it('should successfully get voting count', async () => {
      ...
      expect(queryAdapterServiceMock.getVotingsCount).toHaveBeenCalledWith(
        evmAddress,
      );
    });
  });
