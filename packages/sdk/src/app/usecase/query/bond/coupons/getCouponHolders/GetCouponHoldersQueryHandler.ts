...
export class GetCouponHoldersQueryHandler
  implements IQueryHandler<GetCouponHoldersQuery>
{
  constructor(
  ...
  async execute(
    query: GetCouponHoldersQuery,
  ): Promise<GetCouponHoldersQueryResponse> {
    try {
      ...
      const res = await this.queryAdapter.getCouponHolders(
        securityEvmAddress,
        couponId,
        start,
        end,
