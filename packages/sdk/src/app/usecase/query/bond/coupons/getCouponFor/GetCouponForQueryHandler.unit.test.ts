...
describe('GetCouponForQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetCouponForQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetCouponForQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetCouponForQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying account's coupon: ${errorMsg}`,
        ),
    ...
    it('should successfully get coupon for', async () => {
      ...
      queryAdapterServiceMock.getCouponFor.mockResolvedValue(amount);

      const result = await handler.execute(query);

      expect(result).toBeInstanceOf(GetCouponForQueryResponse);
      expect(result.payload).toStrictEqual(new BigDecimal(amount));
