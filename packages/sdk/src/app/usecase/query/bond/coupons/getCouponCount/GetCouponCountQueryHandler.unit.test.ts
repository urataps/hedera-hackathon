...
describe('GetCouponCountQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetCouponCountQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetCouponCountQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetCouponCountQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying coupon count: ${errorMsg}`,
        ),
    ...
    it('should successfully get coupon count', async () => {
      ...
      expect(queryAdapterServiceMock.getCouponCount).toHaveBeenCalledWith(
        evmAddress,
      );
    });
  });
