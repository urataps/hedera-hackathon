...
describe('GetTotalSecurityHoldersQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetTotalCouponHoldersQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetTotalCouponHoldersQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetTotalCouponHoldersQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying total coupon holders: ${errorMsg}`,
        ),
    ...
    it('should successfully get total coupon holders', async () => {
      ...
      expect(
        queryAdapterServiceMock.getTotalCouponHolders,
      ...
      expect(
        queryAdapterServiceMock.getTotalCouponHolders,
      ).toHaveBeenCalledWith(evmAddress, query.couponId);
    });
  });
});
