  ...
  describe('execute', () => {
    it('throws GetAccountBalanceQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying user balance: ${errorMsg}`,
        ),
    ...
    it('should successfully get account balance', async () => {
      ...
      expect(result.payload).toStrictEqual(
        BigDecimal.fromStringFixed(balanceOf.toString(), security.decimals),
      );
      ...
      expect(queryAdapterServiceMock.balanceOf).toHaveBeenCalledWith(
        evmAddress,
        targetEvmAddress,
      );
    });
