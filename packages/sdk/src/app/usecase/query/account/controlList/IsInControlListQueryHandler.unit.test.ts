...
describe('IsInControlListQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new IsInControlListQueryHandler(
  ...
  describe('execute', () => {
    it('throws IsInControlListQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying user control list status: ${errorMsg}`,
        ),
    ...
    it('should successfully check is in control list', async () => {
      ...
      expect(
        queryAdapterServiceMock.isAccountInControlList,
