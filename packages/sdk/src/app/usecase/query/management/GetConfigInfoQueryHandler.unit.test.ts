...
describe('GetConfigInfoQueryHandler', () => {
  ...
  const resolverAddress = new EvmAddress(
  ...
  beforeEach(() => {
    handler = new GetConfigInfoQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetConfigInfoQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetConfigInfoQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying config info: ${errorMsg}`,
        ),
    ...
    it('should successfully get config info', async () => {
      ...
      queryAdapterServiceMock.getConfigInfo.mockResolvedValue([
        resolverAddress,
        '1',
        1,
      ]);
      ...
      expect(result.payload).toStrictEqual(
        new DiamondConfiguration(resolverAddress, '1', 1),
      );
      ...
      expect(queryAdapterServiceMock.getConfigInfo).toHaveBeenCalledWith(
        evmAddress,
      );
    });
  });
