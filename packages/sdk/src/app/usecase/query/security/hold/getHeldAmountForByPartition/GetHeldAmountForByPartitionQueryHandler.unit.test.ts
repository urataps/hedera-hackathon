...
describe('GetHeldAmountForByPartitionQueryHandler', () => {
  ...
  describe('execute', () => {
    it('throws GetHeldAmountForByPartitionQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying held amount by partition: ${errorMsg}`,
        ),
    ...
    it('should successfully get held amount for by partition', async () => {
      ...
      queryAdapterServiceMock.getHeldAmountForByPartition.mockResolvedValueOnce(
        1,
      );
      ...
      expect(
        queryAdapterServiceMock.getHeldAmountForByPartition,
