  ...
  describe('execute', () => {
    it('throws GetHoldCountForByPartitionQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetHoldCountForByPartitionQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying hold count: ${errorMsg}`,
        ),
    ...
    it('should successfully get hold count for by partition', async () => {
      ...
      queryAdapterServiceMock.getHoldCountForByPartition.mockResolvedValueOnce(
        1,
      );
      ...
      expect(
        queryAdapterServiceMock.getHoldCountForByPartition,
