...
describe('GetHeldAmountForQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetHeldAmountForQueryHandler(
      contractServiceMock,
  ...
  describe('execute', () => {
    it('throws GetHeldAmountForQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetHeldAmountForQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying held amount: ${errorMsg}`,
        ),
    ...
    it('should successfully get held amount for', async () => {
      ...
      expect(queryAdapterServiceMock.getHeldAmountFor).toHaveBeenCalledWith(
        evmAddress,
        targetEvmAddress,
      );
    });
