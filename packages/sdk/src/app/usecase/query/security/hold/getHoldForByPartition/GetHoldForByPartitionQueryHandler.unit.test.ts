...
import {
  GetHoldForByPartitionQuery,
  GetHoldForByPartitionQueryResponse,
} from './GetHoldForByPartitionQuery';
import { GetHoldForByPartitionQueryHandler } from './GetHoldForByPartitionQueryHandler';
import {
  GetHoldForByPartitionQueryFixture,
  HoldDetailsFixture,
  ...
  describe('execute', () => {
    it('throws GetHoldForByPartitionQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetHoldForByPartitionQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying hold: ${errorMsg}`,
        ),
    ...
    it('should successfully get hold for by partition', async () => {
      ...
      queryAdapterServiceMock.getHoldForByPartition.mockResolvedValueOnce(
        holdDetail,
      );
      ...
      expect(
        queryAdapterServiceMock.getHoldForByPartition,
