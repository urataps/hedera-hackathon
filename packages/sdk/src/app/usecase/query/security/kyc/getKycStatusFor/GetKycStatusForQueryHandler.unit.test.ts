...
describe('GetKycStatusForQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetKycStatusForQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetKycStatusForQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetKycStatusForQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying KYC status: ${errorMsg}`,
        ),
    ...
    it('should successfully get kyc status for', async () => {
      ...
      expect(queryAdapterServiceMock.getKycStatusFor).toHaveBeenCalledWith(
        evmAddress,
        targetEvmAddress,
      );
    });
