...
describe('GetKycForQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetKycForQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetKycForQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying KYC data: ${errorMsg}`,
        ),
    ...
    it('should successfully get kyc for', async () => {
      ...
      expect(queryAdapterServiceMock.getKycFor).toHaveBeenCalledWith(
        evmAddress,
        targetEvmAddress,
      );
    });
