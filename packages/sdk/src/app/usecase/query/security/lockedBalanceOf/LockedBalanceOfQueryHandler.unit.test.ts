  ...
  describe('execute', () => {
    it('throws LockedBalanceOfQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        LockedBalanceOfQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying locked balance: ${errorMsg}`,
        ),
    ...
    it('should successfully get lock count', async () => {
      ...
      expect(queryAdapterServiceMock.getLockedBalanceOf).toHaveBeenCalledWith(
        evmAddress,
        targetEvmAddress,
      );
    });
