...
export class GetSecurityHoldersQueryHandler
  implements IQueryHandler<GetSecurityHoldersQuery>
{
  constructor(
  ...
  async execute(
    query: GetSecurityHoldersQuery,
  ): Promise<GetSecurityHoldersQueryResponse> {
    try {
      ...
      const res = await this.queryAdapter.getSecurityHolders(
        securityEvmAddress,
        start,
        end,
      );
