...
export class GetTotalSecurityHoldersQueryHandler
  implements IQueryHandler<GetTotalSecurityHoldersQuery>
{
  constructor(
  ...
  async execute(
    query: GetTotalSecurityHoldersQuery,
  ): Promise<GetTotalSecurityHoldersQueryResponse> {
    try {
      ...
      const res =
        await this.queryAdapter.getTotalSecurityHolders(securityEvmAddress);
