...
describe('IsOperatorQueryHandler', () => {
  ...
  const operatorEvmAddress = new EvmAddress(
    EvmAddressPropsFixture.create().value,
  );
  const errorMsg = ErrorMsgFixture.create().msg;

  beforeEach(() => {
    handler = new IsOperatorQueryHandler(
  ...
  describe('execute', () => {
    it('throws IsOperatorQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying if account is operator: ${errorMsg}`,
        ),
    ...
    it('should successfully check if is operator', async () => {
      ...
      expect(queryAdapterServiceMock.isOperator).toHaveBeenCalledWith(
        evmAddress,
        operatorEvmAddress,
        targetEvmAddress,
      );
