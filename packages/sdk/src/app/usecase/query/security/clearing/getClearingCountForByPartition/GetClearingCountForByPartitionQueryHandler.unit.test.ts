  ...
  describe('execute', () => {
    it('throws GetClearingCountForByPartitionQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetClearingCountForByPartitionQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying clearing count: ${errorMsg}`,
        ),
    ...
    it('should successfully get clearing count for by partition', async () => {
      ...
      queryAdapterServiceMock.getClearingCountForByPartition.mockResolvedValueOnce(
        1,
      );
      ...
      expect(
        queryAdapterServiceMock.getClearingCountForByPartition,
