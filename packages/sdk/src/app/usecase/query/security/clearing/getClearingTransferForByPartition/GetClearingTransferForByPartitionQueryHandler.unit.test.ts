  ...
  describe('execute', () => {
    it('throws GetClearingTransferForByPartitionQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetClearingTransferForByPartitionQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying clearing transfers: ${errorMsg}`,
        ),
    ...
    it('should successfully get clearing transfer for by partition', async () => {
      ...
      accountServiceMock.getAccountInfo.mockResolvedValueOnce(
        accountDestination,
      );

      queryAdapterServiceMock.getClearingTransferForByPartition.mockResolvedValueOnce(
        clearing,
      );
      ...
      expect(
        queryAdapterServiceMock.getClearingTransferForByPartition,
