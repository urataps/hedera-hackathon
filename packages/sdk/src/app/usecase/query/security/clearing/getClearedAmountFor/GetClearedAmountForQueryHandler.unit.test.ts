...
describe('GetClearedAmountForQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetClearedAmountForQueryHandler(
      contractServiceMock,
  ...
  describe('execute', () => {
    it('throws GetClearedAmountForQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetClearedAmountForQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying cleared amount: ${errorMsg}`,
        ),
    ...
    it('should successfully get cleared amount for', async () => {
      ...
      expect(queryAdapterServiceMock.getClearedAmountFor).toHaveBeenCalledWith(
        evmAddress,
        targetEvmAddress,
      );
    });
