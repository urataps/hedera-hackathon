  ...
  describe('execute', () => {
    it('throws GetClearingsIdForByPartitionQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetClearingsIdForByPartitionQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying clearing IDs: ${errorMsg}`,
        ),
    ...
    it('should successfully get clearings id for by partition', async () => {
      ...
      queryAdapterServiceMock.getClearingsIdForByPartition.mockResolvedValueOnce(
        [1],
      );
      ...
      expect(
        queryAdapterServiceMock.getClearingsIdForByPartition,
