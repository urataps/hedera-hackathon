...
describe('GetControlListTypeQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetControlListTypeQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetControlListTypeQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetControlListTypeQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying control list type: ${errorMsg}`,
        ),
    ...
    it('should successfully get control list type SecurityControlListType.WHITELIST', async () => {
      ...
      expect(queryAdapterServiceMock.getControlListType).toHaveBeenCalledWith(
        evmAddress,
      );
    });
