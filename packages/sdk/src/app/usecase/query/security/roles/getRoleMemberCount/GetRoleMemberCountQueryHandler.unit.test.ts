...
describe('GetRoleMemberCountQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetRoleMemberCountQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetRoleMemberCountQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetRoleMemberCountQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying role members count: ${errorMsg}`,
        ),
    ...
    it('should successfully get role count for', async () => {
      ...
      expect(queryAdapterServiceMock.getRoleMemberCount).toHaveBeenCalledWith(
        evmAddress,
        query.role,
      );
    });
