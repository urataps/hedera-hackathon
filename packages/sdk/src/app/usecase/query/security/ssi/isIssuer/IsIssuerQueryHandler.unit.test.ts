...
describe('IsIssuerQueryHandler', () => {
  ...
  const issuerEvmAddress = new EvmAddress(
    EvmAddressPropsFixture.create().value,
  );
  const errorMsg = ErrorMsgFixture.create().msg;

  beforeEach(() => {
    handler = new IsIssuerQueryHandler(
  ...
  describe('execute', () => {
    it('throws IsIssuerQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying issuer status: ${errorMsg}`,
        ),
    ...
    it('should successfully check is issuer', async () => {
      ...
      expect(queryAdapterServiceMock.isIssuer).toHaveBeenCalledWith(
        evmAddress,
        issuerEvmAddress,
      );
    });
