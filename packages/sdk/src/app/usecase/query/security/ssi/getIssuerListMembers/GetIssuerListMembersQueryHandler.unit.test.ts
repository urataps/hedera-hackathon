...
describe('GetIssuerListMembersQueryHandler', () => {
  ...
  const member = new EvmAddress(
    EvmAddressPropsFixture.create().value,
  ).toString();
  const errorMsg = ErrorMsgFixture.create().msg;

  beforeEach(() => {
    handler = new GetIssuerListMembersQueryHandler(
  ...
  describe('execute', () => {
    it('throws GetIssuerListMembersQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetIssuerListMembersQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying issuer list members: ${errorMsg}`,
        ),
    ...
    it('should successfully get issuer list members', async () => {
      ...
      queryAdapterServiceMock.getIssuerListMembers.mockResolvedValueOnce([
        member,
      ]);
      ...
      expect(queryAdapterServiceMock.getIssuerListMembers).toHaveBeenCalledWith(
        evmAddress,
        query.start,
        query.end,
      );
