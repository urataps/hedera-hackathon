...
describe('GetTotalSupplyByPartitionQueryHandler', () => {
  ...
  describe('execute', () => {
    it('throws GetMaxSupplyQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying total supply by partition: ${errorMsg}`,
        ),
    ...
    it('should successfully get total supply by partition', async () => {
      ...
      queryAdapterServiceMock.getTotalSupplyByPartition.mockResolvedValueOnce(
        amount,
      );
      ...
      expect(
        queryAdapterServiceMock.getTotalSupplyByPartition,
