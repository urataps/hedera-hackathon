...
} from './GetMaxSupplyByPartitionQuery';
  ...
  describe('execute', () => {
    it('throws GetMaxSupplyQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetMaxSupplyByPartitionQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying max supply by partition: ${errorMsg}`,
        ),
    ...
    it('should successfully get max supply by partition', async () => {
      ...
      queryAdapterServiceMock.getMaxSupplyByPartition.mockResolvedValueOnce(
        amount,
      );
      ...
      expect(
        queryAdapterServiceMock.getMaxSupplyByPartition,
      ).toHaveBeenCalledWith(evmAddress, query.partitionId);
    });
  });
});
