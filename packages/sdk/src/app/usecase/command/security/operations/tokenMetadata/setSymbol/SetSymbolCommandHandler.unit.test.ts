...
describe('SetSymbolCommandHandler', () => {
  ...
  beforeEach(() => {
    handler = new SetSymbolCommandHandler(
      accountServiceMock,
      transactionServiceMock,
      validationServiceMock,
      contractServiceMock,
    );
    command = SetSymbolCommandFixture.create();
  });
  ...
  describe('execute', () => {
    ...
    it('should successfully set symbol', async () => {
      ...
      transactionServiceMock.getHandler().setSymbol.mockResolvedValue({
        id: transactionId,
      });

      const result = await handler.execute(command);

      expect(result).toBeInstanceOf(SetSymbolCommandResponse);
