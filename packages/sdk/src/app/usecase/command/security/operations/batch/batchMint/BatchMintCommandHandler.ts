...
export class BatchMintCommandHandler
  implements ICommandHandler<BatchMintCommand>
{
  constructor(
  ...
  async execute(command: BatchMintCommand): Promise<BatchMintResponse> {
    try {
      ...
      const res = await handler.batchMint(
        securityEvmAddress,
        bdList,
        evmAddresses,
        securityId,
      ...
      return Promise.resolve(
        new BatchMintResponse(res.error === undefined, res.id!),
      );
    } catch (error) {
