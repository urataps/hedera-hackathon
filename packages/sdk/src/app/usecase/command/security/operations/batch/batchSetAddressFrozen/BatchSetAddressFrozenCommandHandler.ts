...
export class BatchSetAddressFrozenCommandHandler
  implements ICommandHandler<BatchSetAddressFrozenCommand>
{
  constructor(
  ...
  async execute(
    command: BatchSetAddressFrozenCommand,
  ): Promise<BatchSetAddressFrozenResponse> {
    try {
      ...
      const res = await handler.batchSetAddressFrozen(
        securityEvmAddress,
        freezeStatusList,
        evmAddresses,
        securityId,
      ...
      return Promise.resolve(
        new BatchSetAddressFrozenResponse(res.error === undefined, res.id!),
      );
    } catch (error) {
