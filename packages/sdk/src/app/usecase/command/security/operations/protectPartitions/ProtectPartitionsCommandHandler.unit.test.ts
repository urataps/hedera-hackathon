  ...
  describe('execute', () => {
    describe('error cases', () => {
      it('throws ProtectPartitionsCommandError when command fails with uncaught error', async () => {
        ...
        await expect(resultPromise).rejects.toBeInstanceOf(
          ProtectPartitionsCommandError,
        );

        await expect(resultPromise).rejects.toMatchObject({
          message: expect.stringContaining(
            `An error occurred while protecting partitions: ${errorMsg}`,
          ),
    ...
    describe('success cases', () => {
      it('should successfully protect partitions', async () => {
        ...
        transactionServiceMock
          .getHandler()
          .protectPartitions.mockResolvedValue({
            id: transactionId,
          });
        ...
        expect(
          transactionServiceMock.getHandler().protectPartitions,
        ...
        expect(
          validationServiceMock.checkUnprotectedPartitions,
        ).toHaveBeenCalledTimes(1);
        expect(
          validationServiceMock.checkUnprotectedPartitions,
