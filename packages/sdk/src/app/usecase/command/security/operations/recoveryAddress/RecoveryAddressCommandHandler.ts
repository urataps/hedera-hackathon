...
export class RecoveryAddressCommandHandler
  implements ICommandHandler<RecoveryAddressCommand>
{
  constructor(
  ...
  async execute(
    command: RecoveryAddressCommand,
  ): Promise<RecoveryAddressCommandResponse> {
    try {
      ...
      const lostWalletEvmAddress: EvmAddress =
        await this.accountService.getAccountEvmAddress(lostWalletId);
      const newWalletEvmAddress: EvmAddress =
        await this.accountService.getAccountEvmAddress(newWalletId);
      ...
      return Promise.resolve(
        new RecoveryAddressCommandResponse(res.error === undefined, res.id!),
      );
    } catch (error) {
