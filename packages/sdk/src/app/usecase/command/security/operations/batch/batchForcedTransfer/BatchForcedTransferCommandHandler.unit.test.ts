  ...
  describe('execute', () => {
    it('throws BatchForcedTransferCommandError when command fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        BatchForcedTransferCommandError,
      );
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while batch forced transfer: ${errorMsg}`,
        ),
    ...
    it('should successfully batch burn', async () => {
      ...
      transactionServiceMock
        .getHandler()
        .batchForcedTransfer.mockResolvedValue({
          id: transactionId,
        });
      ...
      expect(
        transactionServiceMock.getHandler().batchForcedTransfer,
      ).toHaveBeenCalledTimes(1);
