  ...
  describe('execute', () => {
    describe('error cases', () => {
      it('throws RemoveIssuerCommandError when command fails with uncaught error', async () => {
        ...
        await expect(resultPromise).rejects.toBeInstanceOf(
          RemoveIssuerCommandError,
        );

        await expect(resultPromise).rejects.toMatchObject({
          message: expect.stringContaining(
            `An error occurred while removing issuer: ${errorMsg}`,
          ),
    ...
    describe('success cases', () => {
      it('should successfully remove issuer', async () => {
        ...
        expect(
          validationServiceMock.checkAccountInIssuersList,
        ).toHaveBeenCalledWith(command.securityId, command.issuerId, false);

        expect(
          transactionServiceMock.getHandler().removeIssuer,
        ).toHaveBeenCalledTimes(1);
